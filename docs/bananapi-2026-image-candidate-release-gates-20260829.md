...
1. 固定 45 個英文板型技術目錄、444 個 `.img.xz`、444 個同名 SHA-256 校驗檔及繁體中文、英文候選說明各 45 份。2026-09-07 使用者明確要求逐板英文版；僅此批翻譯作為中文規則的特定例外，繁中原件必須保持不變。
...
6. 依 2026-09-06 混合來源發布工具規則，已驗證候選與新正式目錄可在同一檔案系統共用 inode，切換期間持有固定鎖與相容鎖；發布後不得原地改寫映像。刪除舊外部別名前必須保存逐檔路徑與 inode 證據，確認最新正式檔案仍保留；移除別名不得誤報為回收映像本體空間。
7. 繁體中文說明必須標示 L2／L3 邊界，17 個硬阻擋板型另須明示僅供內部驗證。
