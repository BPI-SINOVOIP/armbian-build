...
- 日期：2026-08-30
- 狀態：完整建置與離線驗證通過；單片 4 GiB 工程實測通過主要硬體路徑
...
最終新映像已完成完整原始碼建置、封裝與唯讀內容驗證。實板測試使用前一版候選，
並在執行中加入與最終來源相同的 Broadcom 韌體別名及第二版 CPU 熱控映射；因此可
證明修正方向與單片 4 GiB 工程樣本的主要硬體路徑，但仍須把最終新映像重新燒錄，
才能完成原生映像開機閉環。
...
```text
/home/pi/log/20260830-m4zero-emac-validation/
```

| 項目 | 結果 | 關鍵數值 |
|---|---|---|
| SD 開機與暖重啟 | 通過 | 三次重啟均到達登入提示 |
| CPU 熱控第二版 | 通過 | 四核心壓力 180 秒，最高 77.336°C，未安全中止 |
| DDR 壓力 | 通過 | `memtester 2800M 1`，18 項成功、0 失敗 |
| eMMC 唯讀 | 通過 | 30 秒，92.5 MiB/s，`err=0` |
| SD 寫入與校驗 | 通過 | 1 GiB，寫入 20.5 MiB/s、校驗讀取 21.1 MiB/s |
| 板載 Wi-Fi | 通過 | 5 GHz、80 MHz、433.3 Mbit/s；雙向 116 至 130 Mbit/s |
| Bluetooth | 基礎通過 | HCI 5.0、錯誤 0、20 秒掃描發現三個裝置 |
| Panfrost GLES | 通過 | OpenGL ES 3.1；3840×2160 完成測試，無 GPU fault |
| Cedrus H.264 | 硬體路徑通過 | 1080p、900 幀、IRQ 增加 900、正常 EOS |
| SHA-256 硬體路徑 | 通過 | `sha256-sun8i-ce` 請求與 IRQ 各增加一次 |
| AC300 EPHY 探測 | 通過 | `dwmac-sun8i` 與 AC300 PHY 綁定成功 |

壓力測試後的最終健康快照顯示：系統已連續運作近三小時、四核心均在線、沒有失敗
服務，也沒有 OOM、記憶體、MMC、Panfrost 或 Cedrus 執行期錯誤。開機時曾記錄
`sun8i-ce: not seeded`，後續單次 SHA-256 測試已由硬體驅動完成，不能把該開機訊息
解讀為 Crypto Engine 不可用。

實測中的初始熱控映射曾升至 99.611°C；第一版 cooling map 仍在 96.371°C
安全中止。最終採用的第二版映射在相同類型壓力下可穩定節流，前兩次失敗紀錄不能
刪除，應保留作為修正依據。

Cedrus 測試證明硬體解碼路徑可用，但 30 秒、900 幀串流耗時約 37.6 秒，約為
23.9 fps，尚不能宣稱 1080p30 即時播放通過；結束時另有一次 GStreamer 參照計數
清理警告。GPU 分數 26 是 3840×2160 的結果，不能與 800×600 分數直接比較。
...
- AC300 目前只有驅動與 PHY 探測證據；沒有接網路線，尚無連線、DHCP、吞吐、
  錯誤計數與斷線重連證據。
...
完成最終映像重燒、EMAC 實體連線、2 GiB／4 GiB 多片冷啟動與外接 I/O 驗證前，
不得將本候選標示為量產或正式發布通過。
