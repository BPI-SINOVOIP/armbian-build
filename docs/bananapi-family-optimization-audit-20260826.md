...
| `bananapim1super` | 開發中 | Banana Pi M1 Super | `rk35xx` | `arm64` | `vendor` | 是 | E | L1 元件可建置 | 先由現行整合提交完成 L1 校準、固定載荷契約及乾淨 L2 重建，再釐清量產無線 BOM、韌體授權與實機功能 |
...
- `bananapim1super`：提交 8c6533a10 的歷史完整映像曾通過內部 L2 軟體守門；現行整合提交尚待重新校準及正式重建，且 SD／eMMC 冷啟動、網路、無線、顯示、GPU、VPU、USB 與 40-pin 仍無實機證據。
