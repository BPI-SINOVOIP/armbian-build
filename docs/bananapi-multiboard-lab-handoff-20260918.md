...
本輪計畫 C／D／E 及續作 F／G／I 所列、能離線完成的受控軟體實作與整合已完成，包含 K3 與 USB／NVMe。
...
I 階段補齊 **R2 十套內部修正候選**：九套新建、一套沿用 F；已完成的 Bookworm XFCE
跨批核對重用，沒有重做。十套最終 XZ 均準備通過，不替換原來源或解除發布限制。
工具提交 `92fd3d32f`；[I 交付紀錄](bananapi-r2-matrix-results-20260919.md)分列來源、驗證及接板界線。
主代理最終重驗十套完整收據、校驗旁檔與重用鏈全部通過；原來源身分及硬體資料庫摘要未變。
[十套完整檔案索引](evidence/bpi-r2-matrix-I-20260918/delivery-I-final.json)提供逐套確切路徑與 SHA-256。

## 範圍

既有目錄共 444 筆映像工作、45 款板型；本輪不重建、替換或刪除原始 XZ。
另有十份 R2 內部衍生候選，不計入或取代這 444 筆原工作。
...
**最新 I 階段：1,535 項全部通過，零失敗、零跳過，173.278 秒。**
新增 74 項方法涵蓋 R2 十套契約、完整逆向摘要、候選／擷取綁定、收據身分、
中斷續作及跨批次重用，包含多組正反向子案例。
G 的 1,461 項、G 初版 1,443 項及 F 的 1,381 項均保留為歷史證據，不將重疊測試相加。命令如下：
...
最新日誌為 `output/evidence/bpi-r2-matrix-I-20260918/lab-final-I-001.log`，SHA-256 為
`d02c874cc17d865fbda36638c2f1a15abdcf5639704eaea9958234adb2719622`。
G 歷史日誌仍為 `output/evidence/bpi-lab-matrix-G-20260918/lab-final-G-fixes.log`，SHA-256 為
`7cc9fb8a64b51ac8773a3a817dc65fba6bc5ecf7572110e796a650ef45e31a76`，當時耗時 183.945 秒。
...
I 階段已依逐套原環境、根 UUID 與固定摘要完成其餘九套候選，沿用上述限制。
五個 OS 的 CLI／XFCE 均已準備通過，不再需要重新建立相同候選。
完整位置、SHA-256 與重用鏈見[I 交付紀錄](bananapi-r2-matrix-results-20260919.md)；
原 G 的十套 R2 仍保持阻擋，不能把新候選當成原檔已修正。
