...
## SD Rootfs PAC Test Flow

The 2026-05-25 SD-rootfs experiment keeps the signed vendor eMMC/UFS boot
chain and changes only the DTBO root argument to mount an Armbian rootfs from
SD. The helper tools are:
...
- DTBO root token: `root=UUID=c43f0ac5-b23c-4797-a0d4-945de5474b37`
- PAC SHA256:
  `a047202e76486a89df40ba88ee5ed6a8f214b44fd07a77900930a944edffb16c`

Validation completed without hardware:

- `dtbo.img` was modified with `fdtput` and verified with `fdtget`.
- `dtbo-sign.img` was regenerated by `sprd_sign`.
- PAC packaging reached `do packet success`.
- `sha256sum -c SHA256SUMS` passed in the PAC work directory.

The remaining gate is hardware boot: flash this PAC to eMMC/UFS, write the
matching `rootfs.ext4` to the SD rootfs partition, then verify `/proc/cmdline`,
`findmnt /`, `lsblk -f`, and storage-related `dmesg` output.
