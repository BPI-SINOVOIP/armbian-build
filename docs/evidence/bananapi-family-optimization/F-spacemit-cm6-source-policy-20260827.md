...
`bananapicm6` 已完成固定來源、SpacemiT K1 啟動鏈、板級 DT、核心功能、韌體追溯與 L2 驗證契約，並已有完整建置及唯讀映像證據。板卡仍保留 `.wip`；目前證據等級為 L2，不得宣稱實機可開機或正式發布。
...
- U-Boot 套件中的 `u-boot.itb` 必須包含 `/extlinux/extlinux.conf`、`/boot/extlinux/extlinux.conf`、K1-X 自動開機環境及 `product_name=k1-x_deb1`。通用驗證器只允許板級政策指定不含路徑的受控載荷名稱，並保持必要與禁止字串的逐字檢查。
