...
上述流程已完成。校準輸出與專用 OverlayFS 上層經掛載、程序、開啟檔案與容器檢查後移除，約回收 `19 GiB`；共用快取下層未修改。正式候選已由提交 `d2cc8559662e69a4b083cedc2efc85c80e26144c` 乾淨重建並通過 L2 即時物質驗證，精確結果以 `I-rockchip-m1super-L2-build-20260828.md` 為準。本文件中的校準雜湊只保留作過程追溯，不再代表現行產物。
