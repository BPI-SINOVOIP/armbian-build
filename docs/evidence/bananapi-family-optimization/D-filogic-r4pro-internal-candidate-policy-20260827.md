...
提交 `18d768b9a` 的第一次專用佇列建置確認 2/2 修補套用且零問題，但 U-Boot 在連結階段發現供應商 defconfig 啟用了未隨最小修補提供實作的 `CONFIG_BOARD_LATE_INIT` 與 `CONFIG_OF_SYSTEM_SETUP`。該失敗產物沒有成為候選；板級最終設定明確停用這兩個鉤子，驗證契約也要求最終 U-Boot 設定保持停用。
