...
本批共收到 17 份 UART 紀錄。排除兩份只有 UTF-8 BOM、沒有任何 UART
內容的空檔後，15 片可判定樣本中有 13 片進入 Linux 使用者空間、2 片失敗：
...
| `450600027` | 無 | 0 | 無法判定 | 檔案只有 3 bytes BOM |
...
- `test1`、`test2` 沒有正式序號。
- `450600027` 的表格註記為「板無 log 輸出」；UART 確實只有 BOM。
...
3. 重新收集 `450600027`、`450601162`，並補齊 `test1`、`test2` 的正式序號。
