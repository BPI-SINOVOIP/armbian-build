...
上述 N0／N1 階段沒有控制電源、操作 UART、重開機、改寫板上 SD／eMMC、啟動 PXE／DHCP／TFTP 服務、重編映像或刪除既有產物。當日追加的 N2 部署另記於文末，不混入前階段結論。
...
使用者改先驗證 M4 Zero、確認 0845，並同意保留既有 `25.2.2`／`6.6.75` 系統的最小部署。授權與範圍先以 `616bc1a4d09ffbd066b4a9b662c346af37e8ff8c` 推送；部署工具及 24 項回歸再以 `ab67ba50de0760d48578ce8332dabf9c2fc63553` 推送，兩次均核對遠端。

原型 161 項回歸、8 組稽核及新工具 24 項回歸通過後，才卸載並排他備份 SD 前 4 MiB，先寫槽 0、再寫 SPL1。`apply-0001` 回讀通過，白名單外前綴、分割表與分割區首尾抽查不變。未對系統分割區發出寫入，並非整卡備份或整卡驗證。

目前卡片未掛載，可交回插入 0845；UART 配對及冷啟動仍待驗，未操作電源。此 SRAM 原型不初始化 DDR、不進 Linux；不將媒體回讀成功記為實板成功。完整範圍、備份、雜湊及復原限制見[最小部署紀錄](bananapi-m4zero-sram-minimal-sd-0845-20260915.md)。
