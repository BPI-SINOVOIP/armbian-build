...
該部署階段結束時卡片未掛載，交回插入 0845；當時 UART 配對及冷啟動仍待驗，未操作電源。此 SRAM 原型不初始化 DDR、不進 Linux；不將媒體回讀成功記為實板成功。完整範圍、備份、雜湊及復原限制見[最小部署紀錄](bananapi-m4zero-sram-minimal-sd-0845-20260915.md)。

## N2 追加：UART 與具名電源首驗

使用者明確指定 `/dev/ttyUSB0` 與 `bpi-pw-1`。先確認串口無其他持有者、依實體路徑綁定並啟動錄製，再讀取共用電源工具交接文件。文件中禁止操作板子插座的文字屬於先前燈具升級工作的限制；本工作依使用者新的板子測試授權執行，不延伸到另一顆插座。

確認正式工具 `0.4.0`，具名查詢通過 MAC 核對且初態為關閉後，執行 `on` 並回讀成功。首次取得救援 `ready`，UART 交握、上載不執行、重新上載並執行均通過。SPL2 成功後只停在 SRAM，不涉及 OS 關機；受控斷電約 10 秒再上電後，SD 槽 0 讀取與執行亦通過。再次斷電重啟後，確認未載入的執行命令被拒絕，新的交握仍成功。

共保存三次 `ready`、兩次插座斷電重啟及七筆具名電源查詢／控制回應；未探索內網、未接觸另一顆插座，未新增 SD／eMMC 寫入。最終插座保持開啟、SPL1 等待命令、UART 已釋放。主代理核對控制追蹤的 nonce 與事件順序，並另委派原始證據及靜態範圍的獨立只讀審查。

完整結果、命令、證據雜湊與不可外推的限制見[0845 實板首驗紀錄](bananapi-m4zero-sram-hardware-0845-20260915.md)。N2 僅記為部分完成，不修改先前離線階段的歷史報告。

獨立審查另發現主機對 smoke 的 SP 只有範圍檢查，未要求 16 位元組對齊；已補上拒絕判定及三個不對齊反例。本輪兩次實收 `00047ff0` 均符合，既有硬體結果不變。修正只在主機工具，不更動固定韌體或 SD；後續離線回歸另存，與先前實板執行來源區分。

## P4B 追加：SPL1 最小升級

2026-09-16 使用者同意一次性升級，先以 `78372c72e` 推送[40 KiB 升級計畫](bananapi-m4zero-spl1-upgrade-20260916.md)。新工具獨立於原兩範圍部署器，只寫固定新版 SPL1；槽 0、分割表與原系統不在本次寫入白名單。

UART 先確認仍在舊 ABI 1 SRAM 救援；核對具名電源身分後關閉，使用者將 SD 插回主機。新舊產物及卡上前 4 MiB 核對通過，自動掛載的分割區正常卸載。完整前置、曾因 MAC 分隔格式而停止的查詢、分割區抽查差異與新備份要求均記錄於升級計畫；尚未取得實板新版結果前，不更新既有 DDR 驗證結論。

新版完整回歸 428 項與 8 組稽核通過後，使用者要求直接燒錄，隨即完成新備份及單一 40 KiB 更新。`apply-0001` 回讀成功，槽 0、其他前綴及本次分割區抽查保持一致；未新增另一輪完整舊版驗證。卡片已釋放，插座保持關閉，待使用者插回 `0845`；不因寫入成功而宣告新版 SRAM 或 DDR 實板通過。新備份、驗證與回讀雜湊見本次升級紀錄。
