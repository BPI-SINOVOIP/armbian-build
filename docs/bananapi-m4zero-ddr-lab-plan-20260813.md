...
狀態：軟體及兩板熱重設矩陣完成；可開機映像與冷開機矩陣執行中
...
0438 已完成 531 組乾淨記錄。480 MHz 保險設定與 792 MHz 單板中心候選各
完成 `M2 10/10`，且所有測試均可恢復 480 MHz。詳細結果與原始證據：

```text
docs/evidence/bananapi-m4zero-opi-ddr/M4ZLAB2-hardware-0438-20260813.md
```

此階段只完成同一次上電內的 watchdog 熱重設掃描。下一個 gate 是第二片板
的相同候選與關鍵邊界交集，之後才進入完整斷電冷開機及 Linux 壓力矩陣。

## 11. 1116 與跨板階段結果

1116 的 480 MHz 保險設定與 792 MHz 候選各完成 `M2 10/10`。兩片板的
792 MHz 合計為 `20/20`，共同已觀察 `TPR6` 零失敗區間為 `0x2e..0x44`，
跨板候選採 `0x3a`。詳細證據：
...
下一個 gate 改為標準 U-Boot 可開機映像、完整斷電冷開機、initrd 搬移驗證
及 Linux 全容量壓力測試。
