...
根檔案系統另保存 `filesystem_label`、`filesystem_labels_complete` 與 `filesystem_label_unique`，供使用根標籤的原廠引導鏈核對。多分割模式須解析全部分割的 ext／FAT 標籤，才可宣告唯一；遇到未辨識分割時，仍可依 UUID 讀取已辨識根系統，但不得據此放行標籤型根配置。唯一性只涵蓋此映像，不代表實板其他 SD、eMMC 或 USB 沒有同名標籤。舊擷取未記錄的標籤不補猜值。
