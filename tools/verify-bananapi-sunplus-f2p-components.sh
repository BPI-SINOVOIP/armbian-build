...
for command in fdtget git jq sha256sum; do
...
[[ "$(git -C "${source_dir}" rev-parse HEAD)" == "${expected_revision}" ]] || fail "BSP 提交不符"

while IFS=$'\t' read -r name relative expected_sha256; do
	path="${source_dir}/${relative}"
	[[ -f "${path}" ]] || fail "缺少 ${name}：${relative}"
	actual_sha256="$(sha256sum "${path}" | cut -d' ' -f1)"
	[[ "${actual_sha256}" == "${expected_sha256}" ]] || fail "${name} 雜湊不符"
done < <(jq -r '.artifacts | to_entries[] | [.key, .value.path, .value.sha256] | @tsv' "${manifest}")

linux_dtb="${source_dir}/$(jq -r '.artifacts.linux_dtb.path' "${manifest}")"
...
uboot_image="${source_dir}/$(jq -r '.artifacts.uboot_image.path' "${manifest}")"
