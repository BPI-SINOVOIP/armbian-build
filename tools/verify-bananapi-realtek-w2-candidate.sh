...
[[ -x "${verifier}" && -x "${checker}" ]] || {
	echo "找不到 BPI-W2 候選驗證器或政策守門器。" >&2
	exit 1
}
[[ -d "${fixed_output_dir}" ]] || {
	echo "找不到 BPI-W2 候選輸出目錄：${fixed_output_dir}" >&2
	exit 1
}
...
extra_status_file="$(mktemp "${repo_dir}/.tmp/bananapi-realtek-w2-status.XXXXXX.json")"
...
export VERIFY_TMP_PREFIX="bananapi-realtek-w2-verify"
...
trap - EXIT
cleanup_extra_status
echo "BPI-W2 ${evidence_level} 內部候選映像通過唯讀守門。"
