...
ROOTFS_LABEL="${ROOTFS_LABEL:-armbian-${RELEASE}-${ROOTFS_FLAVOR}}"
...
usage() {
	...
	Create a BPI-M2C UNISOC vendor-PAC hybrid by replacing the vendor
	System/rootfs.ext4 partition with an Armbian rootfs and repacking PAC.
	...
	Options:
	  ...
	  --rootfs-ext4 PATH    Use an existing ext4 rootfs image directly
	  --label NAME          Output label, default: armbian-<release>-<flavor>
	  --size-mb N           ext4 size for rootfs-tar mode, default: auto
	  --target-root PATH    Default: ${TARGET_ROOT}
	  --no-mkpac            Prepare product directory but do not run mkpac
	  --force               Remove an existing output work directory
	  -h, --help            Show this help

	Environment:
	  SOURCE_ROOT, TARGET_ROOT, ARMBIAN_ROOTFS_CACHE, DATE_TAG, BASELINE,
	  MACHINE, SIGN_PROFILE, RELEASE, ROOTFS_FLAVOR, ROOTFS_LABEL,
	  ROOTFS_SIZE_MB, ROOTFS_TAR, ROOTFS_EXT4, FORCE, KEEP_ROOTDIR, RUN_MKPAC
	EOF
}
...
while (($#)); do
	case "$1" in
		...
		--release)
			...
			ROOTFS_LABEL="armbian-${RELEASE}-${ROOTFS_FLAVOR}"
			;;
		--flavor)
			shift
...
make_ext4_from_rootdir() {
	...
	sudo mkfs.ext4 -F -L rootfs -d "${root_dir}" "${image}" >/dev/null
...
patch_pac_ini() {
	...
	perl -0pi -e "s#\\.\\/out\\/target\\/product\\/${MACHINE}#${product_dir}#g; s#\\.\\/prebuilts#${tree}/prebuilts#g; s#WORK_DIR\\s*=.*#WORK_DIR = ${tree}/#; s#PAC_NAME\\s*=.*#PAC_NAME = ${pac_name}#" "${pac_ini}"
}

patch_pac_ini_name_only() {
	local pac_ini="$1"
	local pac_name="$2"

	perl -0pi -e "s#PAC_NAME\\s*=.*#PAC_NAME = ${pac_name}#" "${pac_ini}"
}
...
main() {
	...
	pac_name="bpi-m2c_${baseline_safe}_${label_safe}_${SIGN_PROFILE}.pac"
	...
	cp -a --reflink=auto "${source_product}" "${product_dir}"

	if [[ -n "${ROOTFS_EXT4}" ]]; then
		rootfs_source="${ROOTFS_EXT4}"
		rm -f "${product_dir}/rootfs.ext4"
		cp -a --reflink=auto "${ROOTFS_EXT4}" "${product_dir}/rootfs.ext4"
		...
		printf 'make ext4 rootfs: %s\n' "${product_dir}/rootfs.ext4"
		rootfs_size="$(make_ext4_from_rootdir "${root_dir}" "${product_dir}/rootfs.ext4" "${ROOTFS_SIZE_MB}")"
	fi
	...
	patch_pac_ini "${pac_ini}" "${tree}" "${product_dir}" "${pac_name}"
	if [[ -f "${pac_ini}.bak" ]]; then
		patch_pac_ini_name_only "${pac_ini}.bak" "${pac_name}"
	fi
	write_build_info "${work_dir}/build-info.txt" "${tree}" "${product_dir}" "${rootfs_source}" "${rootfs_size}" "${pac_name}"
	...
	if [[ "${RUN_MKPAC}" == "yes" ]]; then
		...
		(
			cd "${work_dir}"
			sha256sum "product/cp_sign/${SIGN_PROFILE}/${pac_name}" "product/rootfs.ext4" build-info.txt > SHA256SUMS
		)
	fi
