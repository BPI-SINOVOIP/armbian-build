...
def main() -> None:
    if len(sys.argv) not in (2, 3):
        fail("用法為 check-bananapi-realtek-m4-source-policy.py <契約 JSON> [固定來源目錄]")
    ...
    require(config["full_image_built"] is False, "本契約沒有目前分支的完整映像證據")
    ...
    if config["component_build_completed"]:
        require(config["candidate_level"] == "L1 元件候選", "已保存元件時只能標示 L1 元件候選")
        require(config["candidate_scope"] == "internal-component-only", "元件候選範圍不符")
    ...
    source_argument = Path(sys.argv[2]).resolve() if len(sys.argv) == 3 else None
