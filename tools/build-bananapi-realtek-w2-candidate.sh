...
checker="${repo_dir}/tools/check-bananapi-realtek-w2-source-policy.py"
validation_config="${repo_dir}/config/validation/bananapi-realtek-rtd1296-w2-legacy.json"
fixed_output_dir="${repo_dir}/output/images/2026.08/bananapi-realtek-rtd1296-w2-trixie-legacy-cli"
expected_source_date_epoch=1571768256
expected_cache_lower="/media/pi/SMCI/armbian/bpi-v26.2.1/cache"
expected_cache_target="${repo_dir}/cache"
expected_overlay_root="${repo_dir}/.tmp/bananapi-realtek-w2-candidate-cache-overlay"

requested_output_dir="${OUTPUT_DIR:-}"
if [[ -n "${requested_output_dir}" &&
	"$(realpath -m -- "${requested_output_dir}")" != "$(realpath -m -- "${fixed_output_dir}")" ]]; then
	echo "BPI-W2 只允許固定輸出目錄：${fixed_output_dir}" >&2
	exit 2
fi
...
validate_fixed_overlay_mount() {
	...
	for command in findmnt python3 readlink; do
		command -v "${command}" >/dev/null || {
			echo "BPI-W2 OverlayFS 身分檢查缺少命令：${command}" >&2
			exit 2
		}
	done
...
if len(filesystems) != 1:
    raise SystemExit("BPI-W2 cache 掛載身分不唯一")
mount = filesystems[0]
if os.path.realpath(mount.get("target", "")) != os.path.realpath(sys.argv[2]):
    raise SystemExit("BPI-W2 cache 掛載點不符")
if mount.get("fstype") != "overlay":
    raise SystemExit("BPI-W2 cache 不是 OverlayFS")
options = {}
...
for key, expected in zip(("lowerdir", "upperdir", "workdir"), sys.argv[3:]):
    ...
    if len(actual) != 1 or os.path.realpath(actual[0]) != os.path.realpath(expected):
        raise SystemExit(f"BPI-W2 OverlayFS {key} 身分不符")
PY
	[[ "$(findmnt -no FSTYPE -T "${expected_cache_lower}")" != overlay ]] || {
		echo "BPI-W2 固定 lowerdir 不得是 OverlayFS。" >&2
		exit 2
	}
}

case "${PUBLIC_RELEASE:-no}" in
	1 | true | TRUE | yes | YES) echo "BPI-W2 內部候選禁止公開發布。" >&2; exit 2 ;;
esac
case "${HARDWARE_CLAIMS:-no}" in
	1 | true | TRUE | yes | YES) echo "BPI-W2 內部候選禁止硬體通過聲明。" >&2; exit 2 ;;
esac
[[ "${ALLOW_INTERNAL_W2_CANDIDATE:-no}" == yes ]] || {
	echo "BPI-W2 只允許專用 OverlayFS 入口建立內部候選。" >&2
	exit 2
}
if [[ -n "${SOURCE_DATE_EPOCH:-}" && "${SOURCE_DATE_EPOCH}" != "${expected_source_date_epoch}" ]]; then
	echo "BPI-W2 建置拒絕：SOURCE_DATE_EPOCH 必須是 ${expected_source_date_epoch}。" >&2
	exit 2
fi
validate_fixed_overlay_mount
...
export BOARDS="bananapiw2"
export CANDIDATE_FAMILY_NAME="Realtek RTD1296 W2"
export CANDIDATE_LOCK_FILE=".bananapi-realtek-w2-build.lock"
export MINIMUM_FREE_GIB="${MINIMUM_FREE_GIB:-40}"

[[ -x "${builder}" && -x "${checker}" ]] || {
	echo "找不到 BPI-W2 候選建置器或政策守門器。" >&2
	exit 1
}
