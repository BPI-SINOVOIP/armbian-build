...
write_material_state in_progress "BPI-M1 Super 正式物質驗證執行中"
...
finish_entry_state() {
	...
	if [[ ${exit_status} -ne 0 && "${entry_state_active}" == yes ]]; then
		...
		rm -f "${material_evidence}"
		write_material_state failed "BPI-M1 Super 正式物質驗證失敗"
	fi
	exit "${exit_status}"
}
...
python3 - "${status_file}" "${source_date_epoch}" <<'PY'
import json
import os
import sys
...
if status.get("status") != "complete":
    raise SystemExit("BPI-M1 Super 共用驗證狀態不是 complete")
