...
source_preparer="${repo_dir}/tools/prepare-bananapi-renesas-ai2n-overlay-sources.sh"

PUBLIC_RELEASE="${PUBLIC_RELEASE:-no}" POLICY_ONLY=yes \
	VALIDATION_CONFIG="${validation_config}" "${source_verifier}"

"${source_preparer}"
PUBLIC_RELEASE="${PUBLIC_RELEASE:-no}" REQUIRE_CLEAN_SOURCE_TREES=yes \
	VALIDATION_CONFIG="${validation_config}" "${source_verifier}"
...
echo "AI2N 來源證據與內部候選建置完成：${output_dir}"
